...
X = df['본문']    # okt객체로 만들어서 형태소 분리해야 할 내용
Y = df['말머리']   # 카테고리
encoder = LabelEncoder() # 범주형 데이터를 정수형으로 변경해주는 클래스
labeled_y = encoder.fit_transform(Y) # fit 트랜스폼하면 일반 0, 소식 1, 후기 2, 팁 3, MOD 4로 바뀜
...
# print(df.columns)
# # np.save('../data/dcinside_data_max_{}_wordsize_{}'.format(max, wordsize), xy)
