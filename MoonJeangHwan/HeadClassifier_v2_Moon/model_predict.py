...
df = pd.read_csv('./crawling_data/dcinside_1019.csv')
print(df.head())
df.info()
...
for i in range(len(tokened_x)):
    if len(tokened_x[i]) > 11: # max크기
        tokened_x[i] = tokened_x[i][:12] # 크기가 max크기보다 크면 max크기만큼 자르기
x_pad = pad_sequences(tokened_x, 11) # max크기
